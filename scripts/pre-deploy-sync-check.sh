...
EXPECTED_PROD_REMOTE_HTTPS="https://github.com/frankxai/frankx.ai-vercel-website.git"
EXPECTED_PROD_REMOTE_SSH="git@github.com:frankxai/frankx.ai-vercel-website.git"

resolve_prod_repo() {
  local candidates=()

  if [ -n "${PROD_SYNC_PATH:-}" ]; then
    candidates+=("${PROD_SYNC_PATH}")
  fi

  candidates+=(
    "$(cd "${DEV_REPO}/.." && pwd)/frankx-prod-sync"
    "$(cd "${DEV_REPO}/.." && pwd)/frankx.ai-vercel-website"
  )
  ...
  for candidate in "${candidates[@]}"; do
    ...
    if [ "${remote}" = "${EXPECTED_PROD_REMOTE_HTTPS}" ] || [ "${remote}" = "${EXPECTED_PROD_REMOTE_SSH}" ]; then
      printf '%s\n' "${candidate}"
      return 0
    fi
  done
...
PROD_REPO="$(resolve_prod_repo || true)"
...
if [ -z "${PROD_REPO}" ] || [ ! -d "${PROD_REPO}/.git" ]; then
  echo -e "${RED}ERROR: Production sync clone not found.${NC}"
  echo -e "${YELLOW}Set PROD_SYNC_PATH to a clone whose origin is ${EXPECTED_PROD_REMOTE_HTTPS},${NC}"
  echo -e "${YELLOW}or create one at ../frankx-prod-sync or ../frankx.ai-vercel-website.${NC}"
  exit 1
fi

echo -e "${BLUE}Production sync clone: ${PROD_REPO}${NC}"
