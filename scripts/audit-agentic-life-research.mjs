...
const registryPath = join(ROOT, 'data/research/agentic-life-market.json')
...
check(Array.isArray(registry.systems) && registry.systems.length >= 24, 'registry-depth', `${registry.systems.length} systems; minimum 24`)
check(registry.canonicalUrl === 'https://frankx.ai/research/agentic-life-observatory', 'canonical-url', registry.canonicalUrl)
check(registry.registryUrl.endsWith('/registry.json'), 'registry-url', registry.registryUrl)

const ids = registry.systems.map((system) => system.id)
check(new Set(ids).size === ids.length, 'unique-system-ids', `${new Set(ids).size}/${ids.length} unique`)

const roles = [...new Set(registry.systems.map((system) => system.strategicRole))].sort()
check(requiredRoles.every((role) => roles.includes(role)), 'strategic-role-coverage', `present: ${roles.join(', ')}`)

const categories = [...new Set(registry.systems.map((system) => system.category))].sort()
...
const publicSurface = [registryPath, domainsPath, sourcesPath, pagePath, componentPath]
...
const roleBreakdown = Object.fromEntries(requiredRoles.map((role) => [role, registry.systems.filter((system) => system.strategicRole === role).length]))
const categoryBreakdown = Object.fromEntries(categories.map((category) => [category, registry.systems.filter((system) => system.category === category).length]))
const scoreTotal = registry.systems.reduce(
  (total, system) => total + requiredAxes.reduce((axisTotal, axis) => axisTotal + system.scores[axis], 0),
  0,
)
const possibleScore = registry.systems.length * requiredAxes.length * 3
...
const receipt = {
  ...
  registry: {
    ...
    roles: roleBreakdown,
    categoryBreakdown,
    coveragePercent: Math.round((scoreTotal / possibleScore) * 100),
  },
  checks: {
    total: checks.length,
