...
test('the download endpoint points to the maintained kit and labels the old ZIP as legacy', async () => {
  ...
  assert.match(page, /Two packages\. One clear boundary\./)
  assert.match(page, /Download Operator Kit/)
  assert.match(page, /Download Client Session Kit/)
  assert.match(page, /Keep this package inside Ana&?apos;s practice/)
  assert.match(page, /reviews the exact files and recipient/)
  ...
  assert.match(release, /tag: 'v1\.1\.0'/)
  assert.match(release, /targetCommit: 'faebab8c5ad5cee591b670f39a86163c68cbf745'/)
  assert.match(release, /id: 'operator'/)
  assert.match(release, /id: 'client-session'/)
  assert.match(release, /ana-operator-kit-v1\.1\.0\.zip/)
  assert.match(release, /ana-client-session-kit-v1\.1\.0\.zip/)
  assert.match(release, /Internal to Ana\\'s practice; not for client forwarding\./)
  assert.match(release, /Client-shareable only after Ana or an accountable team member reviews/)
