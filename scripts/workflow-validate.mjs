...
// Builtin / globally-installed agent types (defined by the harness, not as
// repo files) that workflows legitimately reference. These cannot be resolved
// from the filesystem in CI — a clean runner has no ~/.claude/agents — so they
// are allow-listed explicitly. Previously the validator scanned the developer's
// global ~/.claude/agents, which passed locally but failed in CI (non-determinism).
// Add new builtin agent slugs here when a workflow starts referencing one.
...
for (const r of results) {
  ...
  for (const at of (r.agentTypes ?? [])) {
    ...
    if (agentFiles.size > 0 && !agentFiles.has(at) && !agentFiles.has(slug) && !BUILTIN_AGENTS.has(slug)) {
      r.crossRefIssues.push(`agentType "${at}" not found in .claude/agents/ or builtin allowlist (looked for ${at}.md and ${slug}.md)`)
    }
  }
