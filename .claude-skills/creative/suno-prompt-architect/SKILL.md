---
name: suno-prompt-architect
description: Expert Suno AI prompt engineering for cinematic, transformative music creation. Use this skill when creating Suno prompts for Vibe OS sessions, meditation tracks, or any AI-generated music that needs professional quality and emotional resonance.
---

# Suno Prompt Architect
## Professional Music Generation with Suno AI

This skill transforms musical intentions into precisely crafted Suno AI prompts that generate cinematic, emotionally resonant tracks suitable for professional use in Vibe OS sessions, meditation albums, and creator content.

---

## Core Philosophy

**Music as Consciousness Technology**
Every Suno-generated track should serve a transformative purpose - whether energizing a creative session, facilitating meditation, or soundtracking a creator's breakthrough moment.

**Prompt Engineering as Composition**
Writing Suno prompts is like composing a detailed arrangement. Every element matters: genre, mood, instrumentation, production quality, tempo, and energetic intention.

---

## Suno Prompt Anatomy

### The 5-Layer Structure

```
[FOUNDATION] + [MOOD] + [INSTRUMENTATION] + [PRODUCTION] + [INTENTION]
```

**1. FOUNDATION (Genre/Style)**
- Primary genre + fusion elements
- Reference aesthetic periods or movements
- Define rhythmic foundation

**2. MOOD (Emotional Landscape)**
- Core emotion (one word)
- Secondary feeling (supporting)
- Energy level (low/mid/high)

**3. INSTRUMENTATION (Sonic Palette)**
- Lead instruments (2-3 max)
- Rhythmic elements
- Atmospheric textures
- Avoid over-specifying

**4. PRODUCTION (Sonic Quality)**
- Mix characteristics (spacious, intimate, cinematic)
- Effects (reverb, delay, compression feel)
- Frequency balance
- Reference quality level

**5. INTENTION (Energetic Purpose)**
- What transformation should this create?
- Use case scenario
- Desired listener state

---

## Genre Vocabular

y & Fusion Patterns

### Vibe OS Signature Styles

**Conscious Electronic**
```
"Ambient downtempo with spiritual depth"
"Deep house meditation with organic textures"
"Ethereal electronica meets sacred ceremony"
```
- Base: Ambient, Downtempo, Deep House
- Add: Sacred, Ceremonial, Transcendent qualities
- Instrumentation: Synth pads, Tibetan bowls, nature sounds
- Tempo: 60-90 BPM

**Epic Motivation**
```
"Cinematic orchestral hybrid with driving energy"
"Epic trailer music meets modern electronic"
"Motivational orchestral with powerful builds"
```
- Base: Cinematic, Orchestral, Epic
- Add: Electronic elements, modern production
- Instrumentation: Strings, brass, epic drums, synths
- Tempo: 120-140 BPM

**Creative Flow**
```
"Lo-fi hip hop with cosmic ambience"
"Jazzy electronic with flowing progressions"
"Atmospheric trip-hop for deep focus"
```
- Base: Lo-fi, Jazz, Trip-hop
- Add: Cosmic, flowing, evolutionary elements
- Instrumentation: Rhodes, soft beats, atmospheric pads
- Tempo: 80-110 BPM

**Frequency Healing**
```
"432Hz ambient soundscape with binaural tones"
"Solfeggio frequency meditation with crystalline textures"
"Healing frequencies with organic synthesis"
```
- Base: Ambient, Drone, Soundscape
- Add: Specific frequencies (432Hz, 528Hz, 741Hz)
- Instrumentation: Sine waves, crystal bowls, nature
- Tempo: <60 BPM or arhythmic

**Tech Productivity**
```
"Minimal techno for focused work"
"Deep progressive house with hypnotic patterns"
"Intelligent dance music without vocals"
```
- Base: Techno, Progressive House, IDM
- Add: Minimal, hypnotic, evolving qualities
- Instrumentation: Clean synths, precise drums
- Tempo: 120-128 BPM

---

## Mood Mapping System

### Primary Moods & Musical Translation

**Meditative / Reflective**
- Tempo: 60-80 BPM
- Key: Minor modes, Dorian, Phrygian
- Dynamics: Consistent, gentle swells
- Space: Wide, reverberant
- Example: "contemplative and spacious"

**Energizing / Motivating**
- Tempo: 128-145 BPM
- Key: Major, Mixolydian
- Dynamics: Building, crescendos
- Space: Powerful, present
- Example: "uplifting and determined"

**Creative / Flowing**
- Tempo: 90-110 BPM
- Key: Major 7ths, jazz harmony
- Dynamics: Evolving, conversational
- Space: Balanced, intimate
- Example: "exploratory and fluid"

**Healing / Restorative**
- Tempo: <60 BPM or free
- Key: Drones, perfect 5ths
- Dynamics: Minimal variation
- Space: Expansive, oceanic
- Example: "nurturing and gentle"

**Focused / Concentrated**
- Tempo: 120-128 BPM (consistent)
- Key: Modal, minimal harmony
- Dynamics: Stable, hypnotic
- Space: Clear, organized
- Example: "precise and steady"

### Emotional Gradient Words

**Low Energy → High Energy**
- Tranquil → Peaceful → Calm → Contemplative → Serene
- Gentle → Soft → Flowing → Moving → Energetic
- Intimate → Present → Vibrant → Powerful → Intense

**Dark → Light**
- Mysterious → Introspective → Reflective → Hopeful → Bright
- Deep → Rich → Warm → Radiant → Luminous

**Simple → Complex**
- Minimal → Clean → Balanced → Textured → Layered
- Spacious → Open → Full → Dense → Intricate

---

## Instrumentation Vocabulary

### By Function

**Melodic Leaders**
- Piano (acoustic, electric, Rhodes)
- Synth leads (analog, digital, modular)
- Strings (solo violin, string ensemble, cellos)
- Guitar (acoustic, electric, ambient)
- Woodwinds (flute, clarinet, saxophone)

**Rhythmic Foundation**
- Drums (acoustic, electronic, hybrid, hand percussion)
- Bass (upright, electric, sub bass, synthesized)
- Percussion (shakers, congas, tabla, frame drums)

**Atmospheric Textures**
- Synth pads (ambient, evolving, swelling)
- String pads (sustained, lush, cinematic)
- Vocal textures (wordless, ethereal, choir)
- Nature sounds (water, wind, birds, fire)
- Found sounds (field recordings, environmental)

**Sacred/Healing Instruments**
- Singing bowls (Tibetan, crystal)
- Bells (temple, meditation, chimes)
- Gongs (planetary, symphonic)
- Didgeridoo
- Native flutes

### Combination Patterns

**Minimalist (2-3 elements)**
```
"soft piano, ambient pads, gentle rain"
"crystal bowls, binaural tones, distant chimes"
"electric bass, minimal beats, atmospheric synth"
```

**Balanced (4-5 elements)**
```
"acoustic piano, soft strings, light percussion, warm bass, subtle synth"
"Tibetan bowls, bamboo flute, gentle drums, nature ambience, drone"
"electric piano, jazzy bass, brush drums, string pads, vinyl crackle"
```

**Rich (6+ elements)**
```
"orchestral strings, epic brass, cinematic drums, soaring choir, deep bass, atmospheric pads, subtle electronics"
"ambient synths, crystal bowls, temple bells, binaural frequencies, ocean waves, distant thunder, ethereal vocals"
```

---

## Production Quality Descriptors

### Mix Characteristics

**Spatial Qualities**
- Spacious, wide, panoramic, expansive
- Intimate, close, personal, focused
- Cavernous, cathedral-like, reverberant
- Dry, direct, present, immediate

**Frequency Balance**
- Warm (emphasized low-mids, gentle highs)
- Bright (present highs, clear articulation)
- Deep (strong sub bass, foundational lows)
- Airy (open highs, breathing room)
- Full spectrum (balanced across all frequencies)

**Production Style**
- Pristine, crystal clear, hi-fi
- Lo-fi, vintage, analog warmth
- Cinematic, epic, film-quality
- Organic, natural, unprocessed feel
- Modern, polished, radio-ready
- Experimental, textured, layered

**Dynamic Range**
- Compressed, consistent, even
- Dynamic, breathing, natural
- Building, evolving, crescendoing
- Minimal variation, meditative

---

## Tempo & Rhythm Guidance

### BPM Ranges by Use Case

**Meditation & Sleep (0-60 BPM)**
- Free-flowing, arhythmic soundscapes
- Very slow pulse (40-60 BPM)
- Matches resting heart rate
- Example: "slow and drifting"

**Deep Focus & Study (60-80 BPM)**
- Gentle, consistent rhythm
- Alpha brainwave entrainment range
- Non-distracting pulse
- Example: "steady and calm"

**Creative Flow (80-110 BPM)**
- Natural walking pace
- Conversational rhythm
- Evolving patterns
- Example: "flowing and organic"

**Productivity & Energy (110-128 BPM)**
- Motivated, forward motion
- Consistent drive
- Uplifting pace
- Example: "steady and uplifting"

**High Energy & Motivation (128-150 BPM)**
- Powerful, driving force
- Peak performance pace
- Energizing momentum
- Example: "powerful and determined"

**Rhythmic Patterns**
- Straight (4/4, metronomic, consistent)
- Swung (triplet feel, jazz, groove)
- Syncopated (off-beat, complex, engaging)
- Polyrhythmic (layered, evolving, textural)
- Free (rubato, breathing, organic)

---

## Frequency Specifications

When creating healing or meditation tracks, specify exact frequencies:

### Solfeggio Frequencies
- **174 Hz** - Pain reduction, grounding
- **285 Hz** - Healing tissues, regeneration
- **396 Hz** - Liberation from fear, guilt
- **417 Hz** - Facilitating change, undoing situations
- **528 Hz** - DNA repair, miracles, love (most popular)
- **639 Hz** - Connection, relationships, harmony
- **741 Hz** - Awakening intuition, expression
- **852 Hz** - Spiritual order, enlightenment
- **963 Hz** - Divine consciousness, pineal activation

### Concert Pitch Tuning
- **432 Hz** - Natural tuning, cosmic harmony
- **440 Hz** - Standard concert pitch
- **444 Hz** - Crystal clear, cerebral

### Binaural Beat Frequencies
- **Delta (0.5-4 Hz)** - Deep sleep, healing
- **Theta (4-8 Hz)** - Meditation, creativity
- **Alpha (8-14 Hz)** - Relaxed focus, learning
- **Beta (14-30 Hz)** - Active thinking, concentration
- **Gamma (30-100 Hz)** - Peak awareness, higher consciousness

**Example Frequency Specification:**
```
"432Hz tuning with 7.83Hz Schumann resonance binaural beat"
"528Hz solfeggio frequency for heart chakra healing"
"Delta wave binaural tones at 2Hz for deep sleep"
```

---

## Intention Layer (The Magic)

This is what transforms a good track into a transformative experience.

### Intention Frameworks

**For Meditation/Healing:**
```
"Intention: Deep inner peace and cellular healing"
"Intention: Release anxiety and embrace stillness"
"Intention: Activate heart chakra and self-love"
```

**For Creative Sessions:**
```
"Intention: Unlock creative flow and inspired action"
"Intention: Overcome creative blocks with courage"
"Intention: Channel pure creative expression"
```

**For Productivity:**
```
"Intention: Sustained focus and mental clarity"
"Intention: Motivated action without stress"
"Intention: Peak performance and effortless execution"
```

**For Transformation:**
```
"Intention: Personal breakthrough and new beginnings"
"Intention: Courage to take bold action"
"Intention: Awaken to your highest potential"
```

---

## Complete Prompt Examples

### Example 1: Conscious Meditation
```
Ambient electronic meditation, ethereal and transcendent, with crystal singing bowls tuned to 528Hz, soft synth pads, binaural theta waves, gentle wind chimes, spacious cathedral reverb, pristine production, intention: deep inner peace and heart healing, slow and drifting.
```

**Analysis:**
- Foundation: Ambient electronic meditation
- Mood: ethereal and transcendent
- Instrumentation: crystal bowls (528Hz), synth pads, binaural theta, wind chimes
- Production: spacious cathedral reverb, pristine
- Intention: deep inner peace and heart healing
- Tempo: slow and drifting

### Example 2: Epic Motivation
```
Cinematic orchestral hybrid, powerful and uplifting, with soaring strings, epic brass, thunderous taiko drums, modern electronic elements, wide stereo field, film-quality production, intention: courage to pursue bold dreams, building from 120 to 140 BPM with dramatic crescendo.
```

**Analysis:**
- Foundation: Cinematic orchestral hybrid
- Mood: powerful and uplifting
- Instrumentation: strings, brass, taiko, electronics
- Production: wide stereo, film-quality
- Intention: courage to pursue bold dreams
- Tempo: building 120-140 BPM, dramatic crescendo

### Example 3: Creative Flow
```
Lo-fi hip hop with cosmic ambience, exploratory and fluid, featuring warm Rhodes piano, jazzy bass, soft brush drums, vinyl crackle, atmospheric space, intimate mix with analog warmth, intention: unlock creative flow and inspired ideas, steady 95 BPM with evolving textures.
```

**Analysis:**
- Foundation: Lo-fi hip hop with cosmic ambience
- Mood: exploratory and fluid
- Instrumentation: Rhodes, jazzy bass, brush drums, vinyl crackle, atmospherics
- Production: intimate mix, analog warmth
- Intention: unlock creative flow and inspired ideas
- Tempo: steady 95 BPM, evolving textures

### Example 4: Frequency Healing
```
432Hz ambient soundscape for deep healing, nurturing and gentle, with Tibetan singing bowls, soft synth drones, alpha wave binaural tones at 10Hz, oceanic textures, expansive reverb, crystal clear production, intention: cellular regeneration and stress release, arhythmic and free-flowing.
```

**Analysis:**
- Foundation: 432Hz ambient soundscape for healing
- Mood: nurturing and gentle
- Instrumentation: Tibetan bowls, synth drones, alpha binaural (10Hz), ocean
- Production: expansive reverb, crystal clear
- Intention: cellular regeneration and stress release
- Tempo: arhythmic and free-flowing

### Example 5: Tech Productivity
```
Minimal techno for deep focus, precise and steady, with clean analog synths, crisp electronic drums at 124 BPM, subtle bass progression, dry and organized mix, modern polished production, intention: sustained concentration and effortless flow, hypnotic and consistent.
```

**Analysis:**
- Foundation: Minimal techno for deep focus
- Mood: precise and steady
- Instrumentation: analog synths, electronic drums, subtle bass
- Production: dry and organized, modern polished
- Intention: sustained concentration and effortless flow
- Tempo: 124 BPM, hypnotic and consistent

---

## Vibe OS Session Prompt Templates

### Session Pack: "Oracle Frequency"
**Theme:** Business insight + strategic clarity

**Track 1: "Strategic Vision"**
```
Deep progressive house meditation, focused and clear, with organic piano, subtle strings, steady electronic beats at 120 BPM, balanced mix with room to think, pristine production, intention: strategic clarity and confident decision-making.
```

**Track 2: "Executive Flow"**
```
Minimal techno meets ambient, powerful and steady, with precise synths, deep bass, light percussion, spacious production, intention: peak performance and effortless execution, consistent 126 BPM.
```

**Track 3: "Breakthrough Insight"**
```
Cinematic electronic, uplifting and expanding, with soaring pads, gentle piano, building strings, wide stereo field, intention: innovative thinking and bold vision, evolving from 90 to 128 BPM.
```

### Session Pack: "Conscious Creation"
**Theme:** Spiritual creativity + artistic expression

**Track 1: "Sacred Space"**
```
Ambient meditation with sacred textures, peaceful and expansive, featuring crystal bowls at 528Hz, soft chimes, nature ambience, cathedral reverb, intention: creative receptivity and divine inspiration, slow and spacious.
```

**Track 2: "Creative Awakening"**
```
Organic electronic with spiritual depth, flowing and inspired, with warm Rhodes, subtle beats, atmospheric pads, balanced intimate mix, intention: unlock creative channel and authentic expression, 95 BPM.
```

**Track 3: "Artistic Liberation"**
```
Uplifting ambient electronica, joyful and free, with bright synths, gentle percussion, evolving textures, pristine spacious production, intention: fearless creation and artistic courage, building 80 to 120 BPM.
```

---

## Advanced Techniques

### Layering Multiple Intentions
```
"Intention: Release creative blocks (primary) while cultivating self-confidence (secondary) and attracting collaborative opportunities (tertiary)"
```

### Narrative Arc Across Tracks
Design 3-5 track sessions with emotional progression:
1. **Arrival** - Settle into the space
2. **Opening** - Receptivity and presence
3. **Peak** - Core transformation
4. **Integration** - Absorb and embody
5. **Completion** - Grounded and empowered

### Seasonal/Elemental Associations
- **Spring/Air** - Bright, airy, new beginnings, winds
- **Summer/Fire** - Energetic, powerful, passion, warmth
- **Autumn/Earth** - Grounded, rich, harvest, gratitude
- **Winter/Water** - Deep, reflective, cleansing, stillness

### Chakra-Specific Sessions
- **Root** (C) - Grounding, 396Hz, deep drums, earth tones
- **Sacral** (D) - Creative flow, 417Hz, flowing water, warmth
- **Solar Plexus** (E) - Empowerment, 528Hz, fire energy, brightness
- **Heart** (F) - Love, compassion, 639Hz, strings, opening
- **Throat** (G) - Expression, 741Hz, bells, clear communication
- **Third Eye** (A) - Intuition, 852Hz, cosmic sounds, vision
- **Crown** (B) - Divine connection, 963Hz, ethereal, transcendence

---

## Quality Checklist

Before finalizing a Suno prompt, verify:

- [ ] Clear genre/style foundation
- [ ] Specific mood descriptors (2-3 words)
- [ ] Instrumentation details (3-6 elements)
- [ ] Production quality specified
- [ ] Intention clearly stated
- [ ] Tempo or rhythmic feel indicated
- [ ] Appropriate for use case (meditation, work, motivation, etc.)
- [ ] Frequency specifications if healing/meditation
- [ ] Avoids over-specification (let Suno be creative)
- [ ] Reads naturally (not just keyword list)

---

## Common Pitfalls to Avoid

❌ **Too Generic**
"Relaxing music with piano"
- Missing: mood depth, production, intention, specificity

❌ **Over-Specified**
"C major scale piano with exactly 4 chords, reverb at 2.3 seconds, compressor at 4:1 ratio, EQ boost at 3kHz..."
- Too technical, Suno can't control that level of detail

❌ **Contradictory Elements**
"Energizing meditation for deep sleep"
- Conflicting intentions

❌ **Missing Intention**
Technically perfect prompt but no transformative purpose
- Lacks the magic that makes it meaningful

---

## When to Use This Skill

Activate `suno-prompt-architect` when:
- Creating new Vibe OS session packs
- Designing meditation or healing tracks
- Generating background music for content
- Building creator soundtracks
- Developing course module music
- Crafting transformation audio experiences
- Need professional-quality Suno prompts

This skill ensures every Suno-generated track serves its transformative purpose with technical excellence and emotional resonance.

---

**Skill Version:** 1.0
**Last Updated:** November 14, 2025
**Optimized For:** Suno AI v4 and v4.5
**Maintained By:** FrankX Music Team
