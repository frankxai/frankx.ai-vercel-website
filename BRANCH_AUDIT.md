...
| `data/vault-manifest.json` | 11 | Generated by `pnpm vault:scan` (hand-triggered, so legitimately committed) |
...
The single largest conflict source is a file no human edits. `data/route-index.json` and `data/blog-hero-manifest.json` are both rewritten by `prebuild` before `next build` runs, so the committed copies are never read by a deployed build — they exist only to be conflicted over. Removing them from version control would eliminate the top cause of merge friction without changing a single byte of what ships. That change is not made here; it is a separate structural decision.
