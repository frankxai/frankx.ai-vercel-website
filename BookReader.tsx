...
export default function BookReader({
  ...
  // Use Lora for body text (transitional serif optimized for screen reading)
  const bodyFontClass = 'font-book';
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-12 lg:py-20">
          <div className="lg:flex lg:gap-12">
            <article className={`flex-1 max-w-3xl ${isPoetry ? 'mx-auto' : ''}`}>
              ...
              {!chapter.image && (
                <header className="relative space-y-6 mb-12 pb-12 border-b border-white/10">
                  {/* Decorative chapter number */}
                  <div className={`book-chapter-number ${tc.chapterNumberColor || 'text-white'}`}>
                  ...
                  <div className={`book-chapter-label ${tc.badgeText}`}>
                    Chapter {String(chapter.number).padStart(2, '0')}
                  </div>
                  <h1 className={`${fontClass} text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-tight relative z-10`}>
                    {chapter.title}
                  </h1>
