'use client'

import { trackConversion } from '@/lib/gtag'

interface BuyButtonProps {
  href?: string
  label: string
  trackingId?: string
}

export default function BuyButton({ href, label, trackingId }: BuyButtonProps) {
  const handleBuyNow = () => {
    const destination = href || '/contact?product=purchase-interest'
    if (href && trackingId && process.env.NEXT_PUBLIC_GOOGLE_ADS_CONVERSION_ID) {
      trackConversion(href, trackingId)
      return
    }
    window.location.href = destination
  }

  return (
    <div className="text-center">
      <button
        onClick={handleBuyNow}
        className="mt-6 px-6 py-2.5 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white text-sm font-semibold rounded-lg transition-all duration-300 shadow-[0_0_20px_rgba(6,182,212,0.3)] hover:shadow-[0_0_30px_rgba(6,182,212,0.4)] hover:-translate-y-0.5"
      >
        {label}
      </button>
    </div>
  )
}
