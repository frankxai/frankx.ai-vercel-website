...
// ============================================================================
// EXPLORE SECTION — Links to Learn / Create / Tools
// ============================================================================
...
function ExploreSection() {
  const explore = [
    ...
    },
    {
      ...
      desc: 'Suno AI mastery, prompt engineering for music, genre techniques, and production workflows. 500+ tracks and counting.',
      href: '/music/create',
      icon: Sparkles,
    ...
    },
    {
      ...
      desc: '30+ curated apps for learning, practice, theory, sheet music, AI creation, and recording. Every tool a modern musician needs.',
      href: '/music/tools',
      icon: Zap,
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {explore.map((item) => (
            <Link
              key={item.title}
              href={item.href}
              className={`group rounded-2xl border border-white/10 bg-gradient-to-br ${item.accent} p-6 transition-all duration-300 hover:border-white/20 hover:scale-[1.02]`}
            >
              <item.icon className={`h-6 w-6 ${item.textAccent}`} />
              <h3 className="mt-3 text-lg font-bold text-white">{item.title}</h3>
              <p className="mt-2 text-sm text-white/50">{item.desc}</p>
              <p className={`mt-4 text-sm font-medium ${item.textAccent}`}>Explore →</p>
            </Link>
          ))}
        </div>
      </div>
