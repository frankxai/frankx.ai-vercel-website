        ...
        <div className="mx-auto grid max-w-7xl gap-14 px-5 pb-20 sm:px-8 sm:pb-24 lg:grid-cols-[1.08fr_0.92fr] lg:items-end lg:pb-28">
          <div>
            ...
            <p className="mt-7 max-w-2xl text-lg leading-8 text-white/58 sm:text-xl">
              A free vertical product for creators—plus embedded interface studies and the release
              evidence behind every maturity claim.
            </p>
            <div className="mt-9 flex flex-wrap items-center gap-5">
              <TrackedLink
                href="#free-template"
                eventName="v0_primary_cta_clicked"
                eventProperties={{ destination: 'free_template' }}
              ...
              <TrackedLink
                ...
                eventName="v0_secondary_cta_clicked"
                eventProperties={{ destination: 'interface_studies' }}
          ...
          <article className="mt-10 grid overflow-hidden rounded-[2rem] border border-white/10 lg:grid-cols-[0.92fr_1.08fr]">
            <div className="relative min-h-[430px] overflow-hidden bg-[#f2efe6] p-7 text-[#11120f] sm:p-10">
              <div className="flex items-start justify-between gap-5">
                <div>
                  <p className="font-mono text-[10px] tracking-[0.12em] text-[#64645c]">
            ...
            <div className="flex flex-col justify-between bg-[#111214] p-7 sm:p-10">
              <div>
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <span className="rounded-full border border-emerald-400/25 bg-emerald-400/10 px-3 py-1 font-mono text-[10px] tracking-[0.08em] text-emerald-300">
      ...
      <section id="interface-studies" className="border-b border-white/10 py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          <div className="max-w-4xl">
            <p className="font-mono text-[11px] tracking-[0.12em] text-cyan-300/70">
            ...
            <p className="mt-6 max-w-3xl text-base leading-7 text-white/48">
              ...
              Preview one in the embedded device theater, then open the original chat. Each remains
              a reference until it earns source, rights, security, tests, and deployment evidence.
            </p>
          </div>
          <div className="mt-10">
          ...
          <div className="grid gap-10 lg:grid-cols-[0.72fr_1.28fr]">
            ...
            <div className="divide-y divide-white/10 border-y border-white/10">
              {verticalRoadmap.map((item) => (
                ...
                >
                  <span className="font-mono text-[10px] tracking-[0.08em] text-emerald-300/60">
                    {item.stage}
                  </span>
                  <div>
      ...
      <section className="border-b border-white/10 py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          <div className="max-w-4xl">
            ...
            <h2 className="mt-4 font-display text-4xl font-semibold tracking-[-0.035em] sm:text-6xl">
              One kernel. Twenty-two focused businesses.
            </h2>
            <p className="mt-6 max-w-3xl text-base leading-7 text-white/48">
              The shared layer handles identity, content, commerce, delivery, analytics, and
              operations. Each template keeps the audience’s decisive workflow in front.
            </p>
          </div>
          ...
          <div className="mt-10 grid gap-5 lg:grid-cols-2">
            {templatePortfolio.map((lane) => (
              <details
                key={lane.lane}
                className="group overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#101113]"
              >
                <summary className="cursor-pointer list-none p-6 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-emerald-400 sm:p-8">
                  <div className="flex items-start justify-between gap-5">
                    <div>
                      <h3 className="font-display text-2xl font-semibold">{lane.lane}</h3>
                      <p className="mt-3 max-w-xl text-sm leading-6 text-white/45">{lane.promise}</p>
                    </div>
                    <span className="shrink-0 rounded-full border border-white/10 px-3 py-1 font-mono text-[10px] text-white/45">
                      {lane.products.length} builds
                    </span>
                  </div>
                  <p className="mt-5 text-xs font-semibold text-emerald-300 group-open:hidden">
                    Open the build queue
                  </p>
                </summary>

                <ol className="divide-y divide-white/10 border-t border-white/10 px-6 sm:px-8">
                  {lane.products.map((item, index) => (
                    <li key={item.product} className="grid gap-3 py-5 sm:grid-cols-[32px_0.9fr_1.1fr]">
                      <span className="font-mono text-[10px] text-emerald-300/55">
                        {String(index + 1).padStart(2, '0')}
                      </span>
                      <div>
                        <p className="text-sm font-semibold text-white/80">{item.product}</p>
                        <p className="mt-1 text-xs text-white/35">{item.audience}</p>
                      </div>
                      <p className="text-xs leading-5 text-white/45">{item.outcome}</p>
                    </li>
                  ))}
                </ol>
              </details>
          ...
          <div className="mt-16 flex flex-col gap-7 rounded-[2rem] border border-emerald-400/20 bg-emerald-400/[0.05] p-7 sm:p-10 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex shrink-0 flex-wrap items-center gap-3">
              <TrackedLink
                ...
                rel="noreferrer"
                eventName="v0_template_source_clicked"
                eventProperties={{ template: creatorLaunchTemplate.slug, placement: 'closing_cta' }}
              ...
              <TrackedLink
                href="/contact"
                eventName="v0_commission_clicked"
                eventProperties={{ destination: 'contact' }}
