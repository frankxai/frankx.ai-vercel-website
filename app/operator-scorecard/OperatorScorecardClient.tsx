...
// The visitor is being read by the very thing they're scoring themselves on.
// Every transition here is a single camera-like move — GSAP timelines, no
// bounce, always interruptible — never a decorative flourish. Under
// prefers-reduced-motion every stage renders in its resolved, final state:
// no GSAP timeline is ever built, so there is nothing to skip.
export default function OperatorScorecardClient() {
  ...
  const activeHint = activeQuestion?.options.find((o) => o.id === activeHintId)?.hint ?? ''

  const result: ScorecardResult = useMemo(() => scoreScorecard(answers), [answers])

  // ---- Cross-stage transitions ("one camera-like movement") -------------
  // State changes fire immediately and synchronously — never gated behind an
  // animation callback. GSAP only decorates the entrance of whatever just
  // became true, reacting to the state via useGSAP's dependency array below.
  // (An earlier version nested `setStage` inside a GSAP timeline callback;
  // that meant the actual stage change silently depended on the browser's
  // rAF ticker running, which stalls under tab-throttling / hidden
  // visibility — a fragile pattern for a funnel's core navigation.)
  ...
  // ---- Report: beat 4 — the ceiling named with weight, one line at a time
        ...
        <div ref={stageRef}>
          {stage === 'intro' && (
            <div ref={introRef} className="space-y-10 text-center">
              <span
                data-intro-item
                className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-4 py-2 font-mono text-xs uppercase tracking-[0.3em] text-emerald-300"
              >
              ...
              <p data-intro-item className="mx-auto max-w-xl text-base text-white/70 md:text-lg">
                11 questions across the six dimensions that separate a person who prompts well from an
                operator running a real agentic stack.
              </p>
              <div data-intro-item className="flex flex-col items-center gap-4">
                <button
                  type="button"
                  onClick={() => setStage('question')}
                  className="inline-flex min-h-[44px] items-center gap-2 rounded-xl bg-emerald-500 px-8 py-4 text-sm font-semibold text-black transition hover:-translate-y-0.5 hover:bg-emerald-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2 focus-visible:ring-offset-[#05070a]"
                >
          ...
          {stage === 'question' && activeQuestion && (
            <div className="space-y-4">
              ...
              <div ref={questionRef} className="space-y-5 rounded-2xl border border-white/10 bg-white/[0.03] p-7 md:p-9">
                  ...
                  {activeQuestion.options.map((option, idx) => {
                    ...
                    return (
                      <button
                        ...
                        onMouseEnter={() => setActiveHintId(option.id)}
                        onMouseLeave={() => setActiveHintId((prev) => (prev === option.id ? null : prev))}
                        onFocus={() => setActiveHintId(option.id)}
                        onBlur={() => setActiveHintId((prev) => (prev === option.id ? null : prev))}
                        role="radio"
                        aria-checked={isSelected}
                        tabIndex={isSelected || (chosenOptionId === undefined && idx === 0) ? 0 : -1}
                        className={clsx(
                          'w-full min-h-[44px] rounded-xl border border-white/10 bg-white/[0.02] p-4 text-left text-sm transition-[color,background-color,border-color,box-shadow,transform] duration-150 hover:-translate-y-0.5 hover:border-emerald-500/40 hover:bg-white/[0.05] hover:shadow-[0_8px_24px_-12px_rgba(16,185,129,0.35)] focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400',
                ...
                <p className="min-h-[18px] text-xs italic text-white/40" aria-hidden="true">
                  {activeHint}
                </p>
                ...
                <div className="flex items-center justify-between pt-2">
                  <button
                    ...
                    disabled={step === 0}
                    className={clsx(
                      'inline-flex min-h-[44px] items-center gap-1.5 text-sm font-medium text-white/50 transition hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400',
                      step === 0 && 'invisible',
                    )}
                  >
                  ...
                  <button
                    ...
                    className={clsx(
                      'inline-flex min-h-[44px] items-center gap-2 rounded-lg bg-emerald-500 px-6 py-3 text-sm font-semibold text-black transition hover:bg-emerald-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400',
            ...
            <div className="space-y-10">
              ...
              {revealBeat >= 3 && (
                <>
                  ...
                  <div
                    ref={gateBlockRef}
                    className="rounded-2xl border border-emerald-500/25 bg-emerald-500/5 p-7 md:p-9"
                  >
                    ...
                    <form onSubmit={submitEmail} className="mt-6 flex flex-col gap-3 sm:flex-row">
                      ...
                      <input
                        ...
                        className="min-h-[44px] flex-1 rounded-lg border border-white/15 bg-black/30 px-4 py-3 text-sm text-white placeholder:text-white/35 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
                      />
                      <button
                        type="submit"
                        disabled={submitting}
          ...
          {stage === 'report' && (
            <div className="space-y-10">
              ...
              <div className="rounded-2xl border border-amber-500/25 bg-amber-500/5 p-7 md:p-9">
                <div className="font-mono text-xs uppercase tracking-[0.25em] text-amber-300">Your named ceiling</div>
              ...
              <div ref={reportRestRef} className="space-y-10">
                <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-7 md:p-9">
                  ...
                  <div className="mt-6">
                    <DimensionRadar scores={result.dimensionScores} reducedMotion={prefersReducedMotion} />
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center gap-3 pt-2 text-center">
                  <button
                    type="button"
                    onClick={restart}
                    className="min-h-[44px] text-sm font-medium text-white/50 transition hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
                  >
