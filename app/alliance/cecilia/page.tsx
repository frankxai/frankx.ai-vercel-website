  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#090806] text-[#fff8e9]">
      <section className="relative isolate overflow-hidden px-5 pb-16 pt-28 sm:px-8 md:pb-24 md:pt-36 lg:px-12">
        <div className="absolute inset-0 -z-20 bg-[radial-gradient(circle_at_14%_8%,rgba(224,180,125,0.22),transparent_32%),radial-gradient(circle_at_82%_18%,rgba(38,94,76,0.26),transparent_32%),radial-gradient(circle_at_55%_90%,rgba(130,71,103,0.12),transparent_35%),linear-gradient(135deg,#090806_0%,#15110c_48%,#08130f_100%)]" />
        ...
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.08fr_0.92fr] lg:items-center">
          <div>
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <a href="#experience" className="inline-flex min-h-12 items-center gap-2 rounded-full bg-[#f4dfbd] px-6 py-3 text-sm font-semibold text-[#17120d] transition hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ddb87f] focus-visible:ring-offset-2 focus-visible:ring-offset-[#090806]">
          ...
          <aside className="rounded-[2.5rem] border border-white/10 bg-white/[0.05] p-5 shadow-[0_36px_130px_rgba(0,0,0,0.44)] backdrop-blur-2xl sm:p-7">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="font-mono text-xs tracking-[0.18em] text-[#ddb87f]">CECILIA.CHAT</p>
                <h2 className="mt-3 text-3xl font-semibold tracking-tight">The domain is reserved. The product is not live.</h2>
              </div>
              <LockKeyhole className="h-8 w-8 shrink-0 text-[#f1cf99]" aria-hidden="true" />
            </div>
            <dl className="mt-7 divide-y divide-white/10 border-y border-white/10">
              {[
                ...
                <div key={label} className="grid gap-2 py-3.5 sm:grid-cols-[7rem_1fr]">
                  ...
                  <dd className="text-sm leading-6 text-[#fff8e9]/[0.68]">{value}</dd>
                </div>
              ))}
            </dl>
          </aside>
      ...
      <section id="experience" className="scroll-mt-24 px-5 py-16 sm:px-8 md:py-24 lg:px-12">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#ddb87f]">One name, three possible layers</p>
            ...
            <p className="mt-5 text-base leading-7 text-[#fff8e9]/[0.58]">Each layer has a different audience and level of responsibility. The free guide can use approved information only; client and team spaces need clear decisions about identity, access, privacy, and support.</p>
          </div>
            ...
            {experienceLayers.map((layer) => {
              ...
              return (
                <article key={layer.title} className="flex flex-col rounded-[2rem] border border-white/10 bg-white/[0.04] p-6 shadow-[0_24px_90px_rgba(0,0,0,0.28)]">
                  <div className="flex items-start justify-between gap-4">
                    <span className="rounded-full border border-[#ddb87f]/[0.22] bg-[#ddb87f]/[0.07] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#f5dcb4]">{layer.status}</span>
                    <Icon className="h-5 w-5 text-[#f1cf99]" aria-hidden="true" />
                  </div>
                  <h3 className="mt-6 text-2xl font-semibold">{layer.title}</h3>
                  <p className="mt-3 text-sm leading-6 text-[#fff8e9]/60">{layer.detail}</p>
                  <div className="mt-6 border-t border-white/10 pt-4">
                    <p className="flex items-start gap-2 text-xs leading-5 text-[#fff8e9]/[0.43]">
      ...
      <section className="px-5 py-16 sm:px-8 md:py-24 lg:px-12">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.78fr_1.22fr] lg:items-start">
          <div className="lg:sticky lg:top-28">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#ddb87f]">Identity decision</p>
            ...
            <p className="mt-5 text-base leading-7 text-[#fff8e9]/[0.58]">A shared name may help the other women on the team feel represented. It may also dilute Ana's existing reputation. The right answer comes from a short naming and voice workshop, not from a domain purchase.</p>
          </div>
          ...
          <div className="overflow-hidden rounded-[2.2rem] border border-white/10 bg-black/[0.18]">
            {[
              ...
              <article key={title} className={`grid gap-3 p-6 sm:grid-cols-[3rem_1fr] sm:p-7 ${index > 0 ? 'border-t border-white/10' : ''}`}>
                <span className="font-mono text-xs text-[#ddb87f]">0{index + 1}</span>
                <div>
                  <h3 className="text-xl font-semibold">{title}</h3>
                  <p className="mt-2 text-sm leading-6 text-[#fff8e9]/[0.55]">{detail}</p>
                </div>
              </article>
            ))}
          </div>
      ...
      <section className="px-5 py-16 sm:px-8 md:py-24 lg:px-12">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#ddb87f]">Commercial models to discuss privately</p>
            ...
            <p className="mt-5 text-base leading-7 text-[#fff8e9]/[0.58]">Frank can earn a smaller share when he carries real build and operating responsibility. Exact percentages do not belong on a public review page; they belong in an approved agreement with a precise definition of net revenue.</p>
          </div>
          ...
          <div className="mt-10 divide-y divide-white/10 border-y border-white/10">
            {commercialModels.map((model) => (
              <article key={model.label} className="grid gap-4 py-7 lg:grid-cols-[7rem_0.8fr_1.15fr] lg:gap-8 lg:py-9">
                <p className="font-mono text-xs text-[#ddb87f]">{model.label}</p>
                <div>
                  <h3 className="text-xl font-semibold">{model.title}</h3>
                  <p className="mt-3 text-sm leading-6 text-[#fff8e9]/[0.56]">{model.detail}</p>
                </div>
                <div className="rounded-[1.35rem] border border-white/[0.09] bg-white/[0.025] p-4">
                  ...
                  <p className="mt-2 text-sm leading-6 text-[#fff8e9]/[0.62]">{model.bestWhen}</p>
                </div>
              </article>
            ))}
          </div>
      ...
      <section className="px-5 py-16 sm:px-8 md:py-24 lg:px-12">
        <div className="mx-auto grid max-w-7xl overflow-hidden rounded-[2.5rem] border border-white/10 bg-[linear-gradient(135deg,rgba(221,184,127,0.11),rgba(255,255,255,0.025),rgba(25,87,66,0.11))] lg:grid-cols-[0.84fr_1.16fr]">
          <div className="p-6 sm:p-8 lg:p-10">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#ddb87f]">Before money moves</p>
            <h2 className="mt-4 text-3xl font-semibold tracking-tight sm:text-4xl">The agreement comes before the checkout.</h2>
            <p className="mt-5 text-base leading-7 text-[#fff8e9]/[0.58]">Lemon Squeezy, Polar, or Stripe can wait. The product and partnership need a clear owner, support model, data boundary, stop condition, and legal review first.</p>
          </div>
      ...
      <section className="px-5 pb-20 sm:px-8 md:pb-28 lg:px-12">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 border-t border-white/10 pt-8 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex max-w-2xl items-start gap-3">
            ...
            <p className="text-sm leading-6 text-[#fff8e9]/50">Cecilia does not assess or select candidates, provide therapy or legal advice, make hiring decisions, or store client and candidate data on a public page. Those boundaries are part of the product, not footnotes.</p>
          </div>
          <a href={anaLinks.workWithMe} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm font-semibold text-[#f5dcb4] underline decoration-[#ddb87f]/[0.35] underline-offset-4 hover:decoration-[#ddb87f]">
