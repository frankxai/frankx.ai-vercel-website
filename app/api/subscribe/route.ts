...
const LIST_CONFIG: Record<string, { topics: string[] }> = {
  ...
  'agentic-builder-lab': { topics: [TOPICS.newsletter, TOPICS['product-updates']] },
...
async function sendWelcomeEmail(email: string, name: string, listType: string) {
  ...
  if (listType === 'music-lab') {
    ...
    template = ikigaiBrandingEmail({ recipientName: name || 'Creator' })
  } else {
    template = welcomeEmail1({ recipientName: name || 'Creator' })
  }
