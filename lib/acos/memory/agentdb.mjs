...
export async function createAgentDBStore() {
  ...
  const db = new AgentDB({
    dbPath,
    vectorDimension,
    vectorBackend: process.env.AGENTDB_VECTOR_BACKEND || 'rvf',
  })
  await db.initialize()
  ...
  return {
    ...
    async getPatternStats() {
      return getPatternStatsWithoutBlockingExit(bank)
    },

    async close() {
      await closeAgentDB(db)
    },
  }
}

export async function closeAgentDB(db) {
  let vectorCloseError
  try {
    await db.vectorBackend?.close?.()
  } catch (error) {
    vectorCloseError = error
  }
  ...
  try {
    ...
    if (vectorCloseError) {
      throw new AggregateError(
        [vectorCloseError, databaseCloseError],
        'AgentDB cleanup failed',
      )
    }
...
function getPatternStatsWithoutBlockingExit(bank) {
  ...
  try {
    // AgentDB alpha.20 caches stats with a five-minute timer that is not
    // unrefed. Keep its cache behavior without forcing every CLI invocation
    // to stay alive for the full cache window.
