...
export async function getStore() {
  ...
  try {
    ...
    if (_initializing === initializing) _initializing = null
  }
}
...
export async function resetStore() {
  if (_resetting) return _resetting

  const resetting = resetStoreAfterInitialization()
  _resetting = resetting
  try {
    await resetting
  } finally {
    if (_resetting === resetting) _resetting = null
  }
}

async function resetStoreAfterInitialization() {
  const initializing = _initializing
  if (initializing) {
    try {
      await initializing
    } catch {
      // Initialization failures already propagate to their getStore() caller.
    }
  }
