...
export const researchDomains: ResearchDomain[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      {
        title: 'Real-Time Multiplayer Infrastructure (2026)',
        content: 'The Vercel WebSocket limitation is solved. PartyKit (acquired by Cloudflare, October 2025) provides stateful edge servers backed by Durable Objects — 50ms latency to 95% of global users across 300+ data centers. The architecture splits cleanly: Vercel serves the frontend (Next.js, React, Tailwind), while PartyKit handles WebSocket game servers (rooms, state sync, game logic). Cost: $5-10/month for 1,000 concurrent players. This unlocks MOBAs, MMOs, and persistent worlds for browser games.',
        items: [
          { title: 'PartyKit (Cloudflare)', description: 'Stateful edge servers with 50ms global latency. Each game zone = a room (Durable Object). Auto-scales to millions. $5-10/mo for 1K CCU. Apache 2.0 license.', badge: 'Recommended' },
          { title: 'Colyseus', description: 'Open-source Node.js multiplayer framework. Schema-based state sync reduces bandwidth 90%. Host on Fly.io ($14/mo). MIT license. 100K+ CCU with Redis clustering.', badge: 'Open Source' },
          { title: 'Babylon.js (WebGPU)', description: 'Full game engine with 10x rendering speedup via WebGPU Render Bundles. Built-in physics, VR/AR, animation system. Renders 1000+ characters at 60fps with instancing + LOD.', badge: '3D Engine' },
          { title: 'Hathora', description: 'Dedicated game server hosting (14 regions). Best for competitive PvP needing <20ms latency. Docker-based, per-match isolation. WebTransport support.', badge: 'Competitive' },
        ],
      },
      {
        title: 'Building Arcanea: A Browser Fantasy World',
        content: 'The Arcanea world — a mythology-infused 10-Gate progression system with three Academies, Luminor companions, and dual-currency economy — is architecturally feasible as a persistent browser game. The recommended stack: Vercel (frontend) + PartyKit (game server) + Babylon.js (WebGPU rendering). Each of the 10 Gates becomes a zone (PartyKit room), players explore 3D environments rendered at 60fps via WebGPU, and server-authoritative game logic prevents cheating. MVP achievable in a 3-week sprint.',
        items: [
          { title: 'Week 1: Prototype', description: 'PartyKit server (room, player join/leave), Babylon.js 3D scene (WebGPU), basic position sync between players.', badge: 'Sprint' },
          { title: 'Week 2: Optimize', description: 'Delta compression (changed fields only), throttle to 20 updates/sec, client-side prediction, instanced rendering for 100+ players.', badge: 'Sprint' },
          { title: 'Week 3: Deploy', description: 'Deploy PartyKit to Cloudflare, frontend to Vercel. Load test 100+ concurrent players. Basic game logic (movement, interactions).', badge: 'Sprint' },
          { title: 'Architecture', description: '10 Gates = 10 zones (rooms). 3 Academies = faction servers. ARC/NEA currency via Durable Objects SQLite. Luminors = AI NPCs (Inworld AI).', badge: 'Design' },
        ],
      },
    ],
    keyFindings: [
      ...
      'PartyKit (Cloudflare Durable Objects) solves the Vercel WebSocket limitation — 50ms latency, $5/mo, millions of players',
      'Babylon.js with WebGPU Render Bundles delivers 10x speedup — renders 1000+ characters at 60fps in browser',
    ...
    faq: [
      ...
      { question: 'Can you host multiplayer games on Vercel?', answer: 'Yes — all game types are now covered. Turn-based games use Supabase Realtime. Real-time action games (FPS, MOBA, MMO) use PartyKit (Cloudflare Durable Objects) for WebSocket game servers at the edge — 50ms latency globally, $5-10/month for 1K concurrent players. Vercel serves the frontend, PartyKit handles game state.' },
      { question: 'What is PartyKit and how does it solve the WebSocket problem?', answer: 'PartyKit is a serverless game server platform acquired by Cloudflare in October 2025. It runs on Cloudflare Durable Objects — stateful mini-servers at 300+ edge locations globally. Each game room is a Durable Object with its own WebSocket server, compute, and storage. This solves Vercel\'s limitation of no long-lived WebSocket connections. Cost: $0.15 per million requests.' },
      { question: 'Can you build an MMO in the browser?', answer: 'Yes. WebGPU (95% browser support, 10x over WebGL) handles rendering 1000+ characters at 60fps. PartyKit handles networking via zone-based rooms (each zone = a Durable Object). The bottleneck is networking, not rendering — use delta compression and client-side prediction. Production examples: RuneScape, Flyff Universe, Stein.world.' },
