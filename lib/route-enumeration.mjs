 ...
 *
 * @typedef {'core'|'blog'|'workshop'|'product'|'guide'|'library'|'travel'|'os'|'research'|'newsletter'|'partnership'|'tool'|'community'|'section'|'video'|'static'|'legacy'} RouteType
 */
...
const STATIC_ROUTES = [
  ...
  { href: '/cloud', title: 'Cloud AI', description: 'Cloud AI systems for prototypes, AI CoEs, MCP workflows, and production-ready workloads', type: 'core' },
  { href: '/cloud/ai-coe', title: 'AI CoE Operating System', description: 'AI CoE operating system for turning demand into prototypes, field assets, and cloud workload paths', type: 'section' },
  { href: '/cloud/mcp-to-cloud', title: 'MCP-to-Cloud Architecture', description: 'Architecture for connecting AI agents to cloud services, APIs, tools, documents, and governance', type: 'section' },
  { href: '/cloud/prototype-sprints', title: 'AI Prototype-to-Production Sprints', description: '10 business day sprint for working AI prototypes, architecture, demo narrative, and roadmap', type: 'section' },
  { href: '/cloud/gpu-model-architecture', title: 'GPU, Model and Workload Architecture', description: 'AI workload architecture across models, inference, RAG, agents, evaluation, cost, and deployment', type: 'section' },
  ...
  { href: '/llm-hub.json', title: 'LLM Hub JSON', description: 'Agent-readable LLM Hub dataset', type: 'static' },
  ...
  { href: '/music/learn', title: 'Music School', type: 'section' },
  ...
  // Travel — work-and-travel routes (data/travel-journeys.ts). The journey
  // entries contain nested objects, so they are not regex-extractable; the 7
  // detail routes are listed explicitly here (the /travel index is auto-discovered).
  { href: '/travel/barcelona-june', title: 'Barcelona in June', tags: ['travel', 'barcelona', 'spain', 'nomad', 'sonar'], type: 'travel' },
  { href: '/travel/copenhagen-july', title: 'Copenhagen in July', tags: ['travel', 'copenhagen', 'denmark', 'nomad', 'jazz'], type: 'travel' },
  { href: '/travel/edinburgh-august', title: 'Edinburgh in August', tags: ['travel', 'edinburgh', 'scotland', 'nomad', 'fringe'], type: 'travel' },
  { href: '/travel/mexico-city-september', title: 'Mexico City in September', tags: ['travel', 'mexico city', 'mexico', 'nomad'], type: 'travel' },
  { href: '/travel/tokyo-october', title: 'Tokyo in October', tags: ['travel', 'tokyo', 'japan', 'nomad'], type: 'travel' },
  { href: '/travel/lisbon-november', title: 'Lisbon in November', tags: ['travel', 'lisbon', 'portugal', 'nomad', 'web summit'], type: 'travel' },
  { href: '/travel/cape-town-december', title: 'Cape Town in December', tags: ['travel', 'cape town', 'south africa', 'nomad'], type: 'travel' },
...
export function enumerateRoutes() {
  ...
  if (contentIndex?.entries) {
    for (const entry of contentIndex.entries) {
      ...
      if (entry.href.startsWith('/shop/templates/')) continue
      routes.push({
        href: entry.href,
        title: entry.title,
  ...
  // 12. Partnerships
  try {
    const registryRaw = fs.readFileSync(path.join(ROOT, 'content/partnerships/index.ts'), 'utf8')
    ...
    for (const file of files) {
      ...
      if (!registryRaw.includes(`[${slug}.slug]`)) continue
      routes.push({
  ...
  // 14. Family history topics (dynamic route generated from TS registry)
  try {
    const raw = fs.readFileSync(path.join(ROOT, 'lib/familie/geschichte-themen.ts'), 'utf8')
    const topicRegex = /\{\s*slug:\s*['"]([^'"]+)['"][^{}]*?titel:\s*['"]([^'"]+)['"]/gs
    let m
    while ((m = topicRegex.exec(raw)) !== null) {
      const [, slug, title] = m
      routes.push({
        href: `/familie/geschichte/${slug}`,
        title,
        tags: ['familie', 'geschichte', ...tagsFromHref(slug)],
        type: 'section',
      })
    }
  } catch {
  ...
  // 15. Agentic Creator OS docs (dynamic route generated from docs-content.ts)
  try {
    const raw = fs.readFileSync(path.join(ROOT, 'app/products/agentic-creator-os/docs/docs-content.ts'), 'utf8')
    const docKeyRegex = /^\s*['"]([^'"]+)['"]:\s*\{/gm
    let m
    while ((m = docKeyRegex.exec(raw)) !== null) {
      const slug = m[1]
      routes.push({
        href: `/products/agentic-creator-os/docs/${slug}`,
        title: titleFromSlug(slug),
        tags: ['acos', 'docs', ...tagsFromHref(slug)],
        type: 'product',
      })
    }
  } catch {
