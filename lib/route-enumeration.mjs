...
import { BLOG_REDIRECTS, isCanonicalBlogHref } from './blog-redirects.mjs'

const ROOT = process.cwd()
const NOINDEX_TRUE_RE = /\bnoindex\s*:\s*true\b/
const ROBOTS_INDEX_FALSE_RE = /\brobots\s*:\s*\{[\s\S]*?\bindex\s*:\s*false\b/
// These recipient-specific pages remain reachable by their direct URLs, but
// they do not belong in generated search, sitemap, or 404 suggestion surfaces.
// Keep the default closed until a separately reviewed consent record proves
// that the named person opted into public discovery.
const CONSENT_GATED_DISCOVERY_ROUTES = new Set([
  '/friends/ana',
  '/mvu/sabrina',
  '/partnerships/van-ede',
  '/partnerships/proposal/anthropic-2026q2',
])
// These completed-event proposal routes now redirect into the governed MVU
// field record. Keep redirect-only URLs out of search, sitemap, and 404
// suggestions even while their historical page files remain available for
// source recovery.
const RETIRED_DISCOVERY_ROUTE_PREFIXES = [
...
/** Walk a content/ directory and extract frontmatter from approved content extensions. */
function walkMdx(dirRel, basePathFor, extensions = ['.mdx']) {
...
const STATIC_ROUTES = [
  ...
  { href: '/canva', title: 'Canva for Founders', description: 'A governed Canva content operating system connecting the official MCP server, human review, owned media, distribution, and measurable learning', tags: ['canva', 'mcp', 'founder systems', 'content operations'], type: 'core' },
  { href: '/qualities', title: 'Core Qualities', description: 'Freedom, Mastery, Meaning, and Connection — the governing qualities behind FrankX', type: 'core' },
  { href: '/qualities/freedom', title: 'Freedom — The Direction', tags: ['freedom', 'agency', 'autonomy'], type: 'section' },
  { href: '/qualities/mastery', title: 'Mastery — The Method', tags: ['mastery', 'craft', 'practice'], type: 'section' },
  { href: '/qualities/meaning', title: 'Meaning — The Compass', tags: ['meaning', 'purpose', 'coherence'], type: 'section' },
  { href: '/qualities/connection', title: 'Connection — The Multiplier', tags: ['connection', 'belonging', 'collective intelligence'], type: 'section' },
  { href: '/books/great-transition/chapter-05-what-you-own', title: 'What You Own', tags: ['freedom', 'ownership', 'agency'], type: 'section' },
  { href: '/books/self-development/chapter-04-craft', title: 'Craft', tags: ['mastery', 'craft', 'practice'], type: 'section' },
  { href: '/books/self-development/chapter-03-soul', title: 'Soul', tags: ['meaning', 'purpose', 'self-development'], type: 'section' },
  { href: '/books/the-wordless-laws-book-two/the-third-mind', title: 'The Third Mind', tags: ['connection', 'collective intelligence', 'relationship'], type: 'section' },
  { href: '/blog', title: 'Blog', type: 'core' },
  { href: '/journal', title: 'Journal', description: 'Short, dated notes from the build', type: 'core' },
  ...
  { href: '/music-lab', title: 'Music Lab', description: 'Play music on browser instruments, with guided notes and rhythm games.', type: 'core' },
  ...
  { href: '/signals', title: 'Signals', description: 'Source-backed AI architecture notes', type: 'research' },
  { href: '/dream-100', title: 'Dream 100', description: 'Contribution before contact', type: 'research' },
  { href: '/signals/open-reproductions-turn-papers-into-inspectable-work', title: 'Open reproductions turn papers into inspectable work', type: 'research' },
  { href: '/signals/studio-2-closes-the-gap-between-prompt-and-performance', title: 'Studio 2 closes the gap between prompt and performance', type: 'research' },
  { href: '/signals/ultrafast-turns-latency-into-product-architecture', title: 'Ultrafast turns latency into product architecture', type: 'research' },
  { href: '/signals/grok-bot-makes-the-team-channel-an-agent-interface', title: 'Grok Bot makes the team channel an agent interface', type: 'research' },
  { href: '/signals/watermarking-moves-provenance-into-generation', title: 'Watermarking moves provenance into generation', type: 'research' },
  { href: '/signals/regional-inference-makes-geography-an-api-choice', title: 'Regional inference makes geography an API choice', type: 'research' },
  { href: '/signals/daybreak-separates-capability-from-access', title: 'Daybreak separates capability from access', type: 'research' },
  ...
  { href: '/music', title: 'Music', description: 'Listen to FrankX tracks and browse playlists and releases.', type: 'section' },
  { href: '/music-intelligence', title: 'Music Intelligence', description: 'Explore music research, AI agents, tools, and free Suno templates.', tags: ['music', 'intelligence'], type: 'section' },
  { href: '/music-os', title: 'Music OS', description: 'Review track readiness and release workflows in the Music OS cockpit.', tags: ['music', 'os'], type: 'section' },
  { href: '/agents/packs/music', title: 'Music Pack', description: 'Preview the music production agent pack and join its launch waitlist.', tags: ['agents', 'pack', 'music'], type: 'section' },
  ...
  function walk(dir, segments) {
    ...
    if (pageEntry) {
      ...
      try {
        ...
        isRedirectOnly = /\bredirect\s*\(/.test(source)
        sitemap =
...
export function enumerateRoutes() {
  ...
  routes.push(
    ...walkMdx('content/blog', (slug) => `/blog/${slug}`).filter((route) =>
      isCanonicalBlogHref(route.href),
    ),
  )
  ...
  routes.push(
    ...walkMdx(
      'content/newsletters/issues',
      (slug) => `/newsletter/archive/${slug}`,
      ['.mdx', '.md'],
    ),
  )
  ...
  // Reading guides are structured data; include authors and alternate spellings
  // so site-wide search can find them without importing the full book corpus.
  const readingGuides = readJson('data/library-reading-guides.json')
  if (Array.isArray(readingGuides)) {
    routes.push(...readingGuides.map(book => ({
      href: `/library/${book.slug}`,
      title: `${book.title} — ${book.author}`,
      description: book.summary,
      tags: [...book.categories, ...book.aliases],
      type: 'library',
    })))
  }
  routes.push(...extractFromTsRegistry('data/library-collections.ts', 'library', (slug) => `/library/collections/${slug}`))
  ...
  // Topic hubs use the same registry as the public hub pages.
  routes.push(...extractFromTsRegistry('lib/research/hubs.ts', 'research', (slug) => `/research/hubs/${slug}`))
  ...
  try {
    ...
    const objectRegex = /\{\s*slug:\s*['"]([^'"]+)['"],\s*(?:title|name):\s*['"]([^'"]+)['"]/g
    let m
  ...
  // Deduplicate by href. For a blog URL, the canonical MDX record beats a
  // generic app-discovered section; all other routes preserve first-source
  // precedence to avoid unrelated catalog churn.
  const dedupedByHref = new Map()
  for (const r of routes) {
    if (!isCanonicalBlogHref(r.href)) continue
    const current = dedupedByHref.get(r.href)
    const preferCanonicalBlog =
      r.href.startsWith('/blog/') && current?.type === 'section' && r.type === 'blog'
    if (!current || preferCanonicalBlog) {
      dedupedByHref.set(r.href, r)
    }
  }
  const deduped = [...dedupedByHref.values()]
  ...
  return deduped.filter(
    (r) =>
      !CONSENT_GATED_DISCOVERY_ROUTES.has(r.href) &&
      !RETIRED_DISCOVERY_ROUTE_PREFIXES.some(
        (prefix) => r.href === prefix || r.href.startsWith(`${prefix}/`),
      ) &&
      hasAppRoute(r.href),
  )
}
...
export function loadAliases() {
  ...
  if (raw?.aliases && typeof raw.aliases === 'object') {
    ...
    for (const [k, v] of Object.entries(raw)) {
      if (typeof v === 'string' && k.startsWith('/')) aliases[k] = v
    }
  }

  for (const { source, destination } of BLOG_REDIRECTS) {
    if (aliases[source] && aliases[source] !== destination) {
      throw new Error(
        `Conflicting redirect destinations for ${source}: ${aliases[source]} and ${destination}`,
      )
    }
