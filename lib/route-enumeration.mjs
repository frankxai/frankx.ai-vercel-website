...
/**
 * Extract canonical product paths from data/products.ts.
 *
 * Build products use a dynamic route, so the app/ file walk cannot discover
 * their slugs. Keeping `canonicalPath` beside each runtime product record makes
 * the route index follow the same release registry as app/build/[slug].
 */
function extractCanonicalProductRoutes(rel) {
  /** @type {Route[]} */
  const out = []
  try {
    const raw = fs.readFileSync(path.join(ROOT, rel), 'utf8')
    const entryRegex =
      /\{\s*slug:\s*['"]([^'"]+)['"]\s*,\s*canonicalPath:\s*['"]([^'"]+)['"][^{}]*?title:\s*['"]([^'"]+)['"]/gs
    let match
    while ((match = entryRegex.exec(raw)) !== null) {
      const [, slug, canonicalPath, title] = match
      out.push({
        href: canonicalPath,
        title,
        tags: tagsFromHref(slug),
    ...
    /* registry may not exist in some checkouts */
  }
  return out
}
...
export function enumerateRoutes() {
  ...
  // 5b. Build products — canonical routes from the runtime release registry.
  routes.push(...extractCanonicalProductRoutes('data/products.ts'))
