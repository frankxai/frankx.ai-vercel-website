...
Here's the whole argument in 15 lines. This is the provider boundary used throughout the inline guide:
...
### Start with the inline guide

The [First Agent Primer](/guides/first-agent-primer) shows the implementation boundaries in
sequence. It does not currently claim a cloneable starter repository.

### Use the workshop format

The [Build Your First AI Agent workshop page](/workshops/build-first-ai-agent) describes a guided
format for teaching the same six boundaries.

### Read the architectural argument

If you want the reasoning before the implementation, start with
[The six primitives of an AI agent](/blog/six-primitives-ai-agent).

The target artifact is a small research-assistant agent with an explicit tool boundary, bounded
loop, Agent Card, and eval cases. Verify each part in your own project.
...
**Ready to ship?** Two honest paths from here:

- **Public now** — [Use the Six Primitives path](/start-here): read the architectural argument and follow the inline build guide without a form or checkout.
- **Not available** — [Six Primitives Toolkit status](/build/six-primitives-toolkit): a future paid release is under review. Checkout is closed, and final contents, delivery, and terms are not yet asserted.

The paid release remains separate from the public learning path and unavailable until its evidence is complete.
