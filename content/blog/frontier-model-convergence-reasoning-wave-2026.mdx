...
Test-time compute scaling introduces an independent axis: **inference compute budget $C_{\text{infer}}$**. Instead of emitting tokens immediately via greedy argmax sampling, reasoning systems expand a search graph over candidate reasoning trajectories:
