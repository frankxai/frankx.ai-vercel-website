...
```
Sourced → Screened → First Look → Diligence → IC Review → Term Sheet → Closed
```

Each stage transition requires:

- **Screened → First Look**: Score >= 50, human confirms
- **First Look → Diligence**: Score >= 65, DD Lead initiates
- **Diligence → IC Review**: DD report complete, score >= 70
