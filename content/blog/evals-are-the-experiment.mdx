...
The receipts from our [model arena](/research/model-arena) show this split: objective verification outcomes live in the per-task `result` field, blind judge verdicts live in `judgePreference`, and the tally distinguishes mechanical failures from style losses.
...
Concrete examples from our rounds: Opus 4.8 answering a hard reasoning task confidently wrong while Fable 5 solved it. Fable 5 making an unauthorized edit where Opus correctly stopped and asked. A blind style verdict that flipped between rounds—proof that n=1 style judgments shouldn't drive routing.

The deeper catch: output discipline degrading under load. Every model we've tested starts compliant and drifts as task complexity rises. That finding moved "enforce contracts structurally" from preference to doctrine, and we only saw it because we measured at multiple load points with the same rubric.
