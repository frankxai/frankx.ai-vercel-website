...
```mermaid
graph TD
    A[User Goal / Dispatch] --> B[Supervisor Agent / Orchestrator]
    B --> C{Context Compression & State Engine}
    C --> D[Domain Subagent: Code Architect]
    C --> E[Domain Subagent: Verification Sentinel]
    C --> F[Domain Subagent: Research Scout]
    D -->|Tool Call| G[MCP Protocol Mesh]
    E -->|Audit Verdict| H{Quality Gate: PASS / REVISE}
    H -->|PASS| I[Episodic-to-Semantic Memory Vault]
    H -->|REVISE| D
    I --> J[Production Release Artifact]
>>>>>>> origin/main
```
...
- [The 2026 Agentic Hierarchy](/blog/skills-vs-agents-vs-prompts-vs-mcp) — Deconstructing Prompts, Skills, Agents, and MCP.
- [The Agent Skill Standard](/blog/agent-skill-standard-evaluated-workflows-2026) — Writing evaluated, testable SKILL.md packages.
- [MCP in Production](/blog/mcp-protocol-production-architecture-2026) — Building zero-trust tool meshes with JSON-RPC.
- [Subagent Swarm Orchestration](/blog/subagent-swarm-orchestration-fsm-2026) — Defeating state divergence with FSM state graphs.
- [Context Compression & Memory Vaults](/blog/context-compression-memory-vault-systems) — Headroom context compaction and memory tiers.
