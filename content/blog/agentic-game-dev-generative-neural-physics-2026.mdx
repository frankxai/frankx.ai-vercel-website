...
```mermaid
graph TD
    A[Spatial Environment & Player Actions] --> B[Real-Time Game Loop 60 FPS]
    B -->|Event Stream| C[Asynchronous Cognitive Engine 2 Hz]
    C --> D[NPC Episodic Memory & Goal FSM]
    C --> E[Local SLM / LPU Dialogue Generator]
    D --> F[Intent & Action Dispatch]
    E --> F
    F -->|Target Velocity / Dynamic Dialogue| B
    B --> G[Neural Physics & Spatial Audio Renderer]
```
