...
description: "Anthropic's Opus 4.6 brings 1M context, 128K output, and adaptive thinking. Technical breakdown with benchmarks, migration guide, and practical implications for builders."
date: "2026-02-06"
...
> **Correction (March 31, 2026)**: The original article stated Opus 4.6 introduced a 67% price cut. This was incorrect — the $5/$25 pricing was established with Opus 4.5 (the cut was from Opus 4.0's $15/$75). Opus 4.6 maintains the same pricing while adding significant capability improvements. The 1M context window is now generally available. Article updated accordingly.

**TL;DR**: Anthropic released Claude Opus 4.6 on February 5, 2026. The headline numbers: 1M token context window (now GA), 128K output tokens, and adaptive thinking that replaces manual budget tuning. Pricing stays at $5/$25 per million tokens — same as Opus 4.5. ARC-AGI-2 jumped from 37.6% to 68.8%. There are breaking changes — prefilling assistant messages now returns a 400 error, and `budget_tokens` is deprecated. Here's what actually matters for builders.
...
2. **The pricing stays accessible** — at $5/$25 per million tokens (same as Opus 4.5, which dropped 67% from Opus 4.0's $15/$75). Opus costs 1.67x Sonnet — meaning the capability gap with 4.6 is massive while the cost premium remains modest.
...
The 1M context window launched in beta and is now generally available. The MRCR v2 benchmark at 76% confirms the model can actually use that context effectively — not just accept it.
...
| -------------- | --------- | ---------- | ------------------ |
| Opus 4.0 (old) | $15.00    | $75.00     | 5.0x               |
| Opus 4.5       | $5.00     | $25.00     | 1.67x              |
| **Opus 4.6**   | **$5.00** | **$25.00** | **1.67x**          |
| Sonnet 4.6     | $3.00     | $15.00     | 1.0x               |
| Haiku 4.5      | $1.00     | $5.00      | 0.33x              |

The pricing story here is value, not cuts. Opus 4.6 delivers substantially more capability at the same $5/$25 price point established by Opus 4.5. The 67% reduction happened in the 4.0→4.5 transition. What 4.6 adds is a massive capability leap — 1M context, 128K output, adaptive thinking — without any price increase.

At 1.67x the cost of Sonnet, the calculation for "is this task worth Opus?" favors Opus for any task requiring deep reasoning, long context, or high-stakes accuracy.
...
2. **Cross-Modal Coordination**: With 1M context, GenCreator can hold an entire content brief (target keywords, brand voice, existing blog archive, visual asset inventory) while generating. The model doesn't lose context switching between writing prose, crafting image prompts, and structuring metadata.

3. **Quality Gating**: Adaptive thinking means the quality review agents auto-calibrate their depth. A quick SEO check uses `low` effort; a full editorial review with fact-checking uses `high`. No more manually configuring reasoning budgets per pipeline stage.
...
_Analysis by Frank — AI Architect, builder of the Agentic Creator Operating System.
Published February 6, 2026 with validated benchmarks from official Anthropic documentation and independent sources._
