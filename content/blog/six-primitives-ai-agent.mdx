...
<Callout type="tip">
**The workshop format.** The workshop page describes a guided version of the same six-part build. This post remains the written architectural reference.
</Callout>
...
The naive view: "a model is where the AI lives." The architectural view: **a model is a pluggable component behind an interface.** In the reference implementation below, that interface is five lines:
...
The [Agent2Agent (A2A) protocol](https://a2a-protocol.org/latest/specification/) defines an "Agent Card"—a JSON document describing identity, supported interfaces, capabilities, security requirements, media types, and skills. A public Card is discoverable at `/.well-known/agent-card.json` on your agent's domain.

Here's an illustrative Agent Card for the same research-assistant example:
...
```json
{
  ...
  "provider": { "organization": "Frank Riemer", "url": "https://frankx.ai" },
  "version": "0.1.0",
...
The Card is a public interoperability contract, not an implementation description. Declare only interfaces and skills your server actually supports, and validate against the exact protocol version in each interface.

### The Oracle Open Agent Specification

The [Oracle Open Agent Specification (Agent Spec)](https://oracle.github.io/agent-spec/) is a portable, declarative language for describing agents and structured workflows. Compatible runtimes and adapters can interpret those definitions; that is a different contract from A2A discovery.

The two specifications can complement each other, but neither guarantees an automatic mapping to the other. Treat any generated Agent Card as release code and verify it against the endpoint that actually runs.
...
- **Inspect the guided format** — [Build Your First AI Agent workshop](/workshops/build-first-ai-agent)
...
The six primitives are the map. The guide shows the implementation boundaries inline.

---

**Want the shortest public sequence?** [Use the Six Primitives path](/start-here) — the essay first, then the inline build guide. No form and no checkout.
