...
An intelligence index, a repository benchmark and a workflow test measure different things. The [model hub](/llm-hub) keeps those columns separate and identifies the publisher of each result. None is a substitute for your own acceptance set.
...
Use the [interactive calculator on the LLM hub](/llm-hub) to test assumptions. It covers standard short context and excludes cache creation, processing-tier adjustments and taxes. Equal token counts across providers are only a planning convenience because tokenizers differ.
