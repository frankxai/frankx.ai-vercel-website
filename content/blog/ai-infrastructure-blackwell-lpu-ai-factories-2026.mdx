...
Measuring infrastructure purely by the hourly rental rate of a GPU ($/hour) is an amateur financial model. Production AI economics are governed by the **Cost Per Verified Outcome (CPVO)**:

$$\text{CPVO} = \frac{\text{Hourly Compute Cost} \times \text{Execution Latency (Hours)}}{\text{Deterministic Verification Pass Rate}}$$
