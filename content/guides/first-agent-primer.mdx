...
description: "The canonical long-form guide to building your first AI agent. Covers the six primitives, a working TypeScript build on Vercel AI SDK, an A2A 1.0 Agent Card, and three eval cases you can run today."
date: "2026-04-24"
author: "Frank"
category: "AI Agents"
...
This guide shares the same six-part curriculum as the [Build Your First AI Agent workshop format](/workshops/build-first-ai-agent), but keeps every implementation boundary visible on the page.

**By the end of this guide you will have:**

- A working research-assistant agent running on your laptop
- The same agent deployed to a public Vercel URL
- A valid A2A 1.0 Agent Card served at `/.well-known/agent-card.json`
...
## Part 3 — Prepare a project you control

Use an existing Next.js TypeScript project that you can safely change. This guide does not currently
claim a cloneable starter repository; the implementation that follows is shown inline so you can
review each boundary before adding it to your own project.
...
Before modifying anything, read the six primitives as they appear in the reference implementation:
...
### Spec (public/.well-known/agent-card.json)

An A2A 1.0 Agent Card—JSON describing the agent's identity, supported interfaces, skills, and security boundary. See the [Agent Card guide](/guides/agent-card-a2a-spec) for field-by-field coverage.

### Deploy (next.config.mjs + vercel.json)

`vercel --prod` puts the agent on the public internet. Publishing the Agent Card at the well-known URL makes its location predictable to compatible A2A clients. CORS headers are a separate browser boundary: they allow browser-based clients from approved origins to fetch the Card, but they do not create discovery or make a client A2A-compatible.

---

## Part 5 — Build your own agent

The inline reference is a research assistant. To make the pattern yours, make three changes:
...
The inline reference has one tool (`web_search`). Add a second one specific to your domain. Examples:
...
Add an Agent Card at `public/.well-known/agent-card.json`. This minimal public example declares one HTTP+JSON interface and no authentication scheme:
...
```json
{
  ...
  "supportedInterfaces": [
    {
      "url": "https://your-vercel-url.vercel.app/a2a/v1",
  ...
  "skills": [
    {
      ...
      "description": "Researches a question and returns a cited response.",
...
- Validate it with the SDK for your declared protocol version and the [current A2A specification](https://a2a-protocol.org/latest/specification/)
...
At this point, require all three eval cases to pass, validate the deployed Agent Card, and confirm
the public endpoint responds before choosing a next branch. The [workshop's six branches](/workshops/build-first-ai-agent)
describe possible next areas:
...
Each branch re-implements the same research-assistant agent on a different stack. The Agent Card contract can stay stable only if every branch exposes the same declared A2A interface; a stack without native support needs a compatible binding or adapter.
...
1. Every request is re-sending a huge system prompt. Check the current provider and SDK documentation for supported caching behavior.
2. `maxSteps` is too high and the agent is looping. Cap at 5 for a simple agent.
3. Session memory is unbounded. Set an explicit retention bound in your own `memory.ts`.

### "A browser on another origin cannot fetch the Agent Card"

That is a browser CORS boundary, not a discovery failure. Configure the required origin policy explicitly for `/.well-known/agent-card.json`; do not assume a template has done it for you. Server-to-server clients are not governed by browser CORS.
...
This guide is CC-BY-NC 4.0 licensed.
