...
The Agent Card is one half of the portability story. The other half is the [Oracle Open Agent Specification (OAS)](https://docs.oracle.com/) — a framework-agnostic YAML/JSON spec that describes **the agent's internals** (tools, memory, workflows), not just its external interface.
