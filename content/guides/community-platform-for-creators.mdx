...
2. **Official MCP** exposes vendor-documented tools to an external AI client. The dated registry currently contains **16 documentation-verified vendor-supported MCP surfaces**—but they range from mature hosted services to read-only betas and an experimental local bundle.
...
1. **Execution surfaces** — community, courses, events, email, support, mobile and future custom experiences.
2. **Stable community gateway** — one owned vocabulary for member identity, entitlements, activity, drafts, approvals, actions and exports. Vendor adapters translate it to each product.
3. **Governed intelligence** — AI can read within a member or operator’s permissions, reason privately, propose a command, cross a human or policy gate, act through a scoped tool, and leave an audit record.
4. **Owned data plane** — canonical member IDs, consent, roles, entitlements, relationships, outcomes, vendor-object mappings and event history in an owned database.
...
[Open the proposed Starlight Community OS blueprint](/downloads/starlight-community-os-blueprint.md) for the reference schema, adapter contract, AI approval envelope, 90-day implementation sequence, production objectives and exit drill.
...
[Open the vendor due-diligence checklist](/downloads/community-platform-vendor-due-diligence-checklist.csv) for 36 contract, API, AI, security, mobile, economics and exit tests. Use it during the demo and require evidence before the order form—not after the migration.
