...
This guide now includes a small, inspectable [Agentic Obsidian Second Brain Starter Kit](/downloads/agentic-obsidian-second-brain-starter-kit-v1.zip). It is intentionally made of Markdown rather than a proprietary vault export:

- `VAULT.md` operating contract — purpose, structure, permissions, provenance, and review rules.
- `obsidian-vault-steward/SKILL.md` — safe retrieval, capture, linking, and maintenance procedure.
- `session-close.md` — decisions, evidence, changed artifacts, open questions, and next start.
- `ACCEPTANCE-TEST.md` — a practical gate for retrieval, bounded writes, provenance, and recovery.
...
Do not judge readiness by whether the agent produced one impressive summary. Use a repeatable acceptance gate. The [starter kit](/downloads/agentic-obsidian-second-brain-starter-kit-v1.zip) includes the full acceptance test; the minimum bar is:
