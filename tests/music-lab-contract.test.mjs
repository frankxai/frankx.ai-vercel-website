...
const guitarEngine = read('lib/music-lab/guitar/engine.ts')
const playbackTimeline = read('lib/music-lab/playback-timeline.ts')
...
test('guided guitar renders real six-string tabs with adjustable reference playback', () => {
  ...
  assert.match(guitarTabs, /Resume phrase/)
  assert.match(guitarTabs, /Tempo/)
  assert.match(guitarTabs, /fret/)
  assert.match(guitarTabs, /PlaybackTimeline/)
  assert.match(guitarTabs, /setRate\(nextTempo \/ 60000\)/)
  assert.match(guitarTabs, /music_lab_instrument_played/)
  assert.match(guitarTabs, /music_lab_lesson_selected/)
  assert.match(guitarEngine, /new AudioContext\(\{ latencyHint: 'interactive' \}\)/)
  assert.match(guitarEngine, /private graphNodes/)
  assert.match(guitarEngine, /activeVoiceCount/)
  assert.match(guitarEngine, /await context\.close\(\)/)
  assert.match(playbackTimeline, /private timer: unknown = null/)
...
test('piano paths keep sampled audio, a fallback voice, and responsive keyboard controls', () => {
  ...
  assert.match(guidedPiano, /music_lab_instrument_played/)
  assert.match(violinComponent, /music_lab_take_recorded/)
  assert.match(violinComponent, /music_lab_take_replayed/)
})
