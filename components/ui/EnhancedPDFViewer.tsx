...
import { getSessionId, trackPDFView, trackPDFDownload } from '@/lib/client-analytics'
import PDFEmailModal from './PDFEmailModal'
import DownloadCounter from '../guides/DownloadCounter'
...
export default function EnhancedPDFViewer({
  ...
  const handleDownload = useCallback(() => {
    // Track download
    trackPDFDownload(guideSlug, title, 'direct')
  ...
  return (
    <>
      <div className={`relative w-full ${className}`}>
        ...
        {showDownloadCounter && (
          <div className="mb-4">
            <DownloadCounter guideSlug={guideSlug} />
          </div>
        )}
