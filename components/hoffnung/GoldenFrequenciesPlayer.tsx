...
import type { GoldenTrack } from '@/data/hoffnung-golden-tracks'

export type GoldenFrequencyTrack = GoldenTrack & { streamUrl?: string }

const UNAVAILABLE_MESSAGE = 'Wiedergabe gerade nicht möglich. In der Suno-Playlist unten anhören.'

export function GoldenFrequenciesPlayer({ tracks }: { tracks: GoldenFrequencyTrack[] }) {
  ...
  return (
    <section className="py-24 md:py-32 px-6">
      ...
      <p role="status" className="sr-only">
        {failedIndex === null ? '' : `${tracks[failedIndex]?.title}: ${UNAVAILABLE_MESSAGE}`}
      </p>
          ...
          return (
            <ScrollReveal key={track.sunoId} delay={i * 0.05}>
              <button
                ...
                className={`w-full text-left disabled:pointer-events-none disabled:opacity-60 glass-card-dawn p-4 md:p-5 flex items-center gap-4 group transition-all duration-300 ${
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {(failedIndex === i || !track.streamUrl) && (
                    <p className="mt-1 text-xs text-amber-200/60">{UNAVAILABLE_MESSAGE}</p>
                  )}
