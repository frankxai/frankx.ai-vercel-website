...
export default function QualityFrame({ active, step = 4 }: { active?: QualitySlug; step?: number }) {
  const beamClass = (visible: boolean) =>
    `absolute bg-gradient-to-r from-amber-100/45 via-white/22 to-cyan-200/45 transition-[opacity,transform] duration-500 motion-reduce:transition-none ${
      visible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'
    }`

  return (
    <div className="relative aspect-square w-full max-w-[29rem]" aria-hidden="true">
      <div className="absolute inset-[12%] bg-[#0d0d0e]/75 shadow-[0_0_80px_rgba(16,185,129,0.08)]">
        ...
        <div className={`absolute inset-[23%] border bg-[#090909] transition-colors duration-500 motion-reduce:transition-none ${step >= 4 ? 'border-emerald-200/20' : 'border-white/10'}`}>
          ...
          <div key={node.slug}>
            <div
              className={`absolute ${node.line} transition-colors duration-300 motion-reduce:transition-none ${
                selected ? 'bg-emerald-300/35' : 'bg-white/8'
              }`}
            />
            <div
              className={`absolute ${node.position} rounded-full border px-3 py-1 font-mono text-[9px] uppercase tracking-[0.18em] transition-colors duration-300 motion-reduce:transition-none ${
                selected
                  ...
                  : 'border-white/10 bg-[#0a0a0b] text-white/50'
              }`}
            >
              {node.label}
            </div>
