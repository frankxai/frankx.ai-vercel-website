...
export default function BlogFeedback({
  ...
  const storageKey = `blog-feedback-${postSlug}`;
  ...
  // Fetch aggregate
  useEffect(() => {
    fetch(`/api/feedback?type=blog&slug=${postSlug}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  return (
    <div className="text-center py-6 space-y-4">
      ...
      {aggregate && aggregate.total >= 10 && !showComment && (
        <p className="text-white/25 text-xs">
          {Math.round((aggregate.positive / aggregate.total) * 100)}% of readers found this helpful
        </p>
      )}
    </div>
  );
