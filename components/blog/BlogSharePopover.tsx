...
interface BlogSharePopoverProps {
  postTitle: string;
  postSlug: string;
}
...
export default function BlogSharePopover({
  ...
  const postUrl = `https://frankx.ai/blog/${postSlug}`;
  ...
  const handleShareX = () => {
    ...
    const url = SHARE_URLS.twitter(tweetText, postUrl, 'frankxeth');
    window.open(url, '_blank', 'noopener,noreferrer,width=600,height=400');
    setVisible(false);
  };

  const handleShareLinkedIn = () => {
    const url = SHARE_URLS.linkedin(postUrl);
    window.open(url, '_blank', 'noopener,noreferrer,width=600,height=400');
    setVisible(false);
  };
