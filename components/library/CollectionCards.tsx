    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {libraryCollections.map(collection => {
        ...
        return <Link key={collection.slug} href={`/library/collections/${collection.slug}`} prefetch={false}
          ...
          <div className="mb-5 flex h-32 items-end gap-3 overflow-hidden" aria-hidden="true">
            {featured.map((book, i) => <BookCover key={book.slug} title={book.title} author={book.author} src={book.cover} imageAlt={book.coverAlt} hasGuide={book.hasGuide} className={`w-20 ${i === 1 ? 'mb-2' : ''}`} />)}
          </div>
