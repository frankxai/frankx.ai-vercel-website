...
export function BookCover({ title, author, src, imageAlt, className = '', priority = false }: {
  ...
  return (
    <div className={`relative aspect-[2/3] shrink-0 overflow-hidden rounded-md border border-white/15 bg-[#12252a] ${className}`}>
      <div className="absolute inset-0 flex flex-col justify-between p-[12%] text-left" role="img" aria-hidden={Boolean(src && src !== failedSource)} aria-label={`${title} by ${author} — Library bookplate`}>
        <span className="text-[8px] leading-tight tracking-wider text-emerald-200">FrankX Library</span>
        <span className="break-words text-[clamp(9px,1.1vw,14px)] font-semibold leading-snug text-white">{title}</span>
        <span className="border-t border-emerald-200/25 pt-2 text-[8px] leading-tight text-emerald-100/80">{author}</span>
      </div>
