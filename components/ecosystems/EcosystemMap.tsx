...
import { startMapLifecycle } from '@/lib/map-lifecycle'
import mapAssets from '@/lib/maplibre-assets.json'
...
const MapComponent = ({ spaces }: { spaces: Space[] }) => {
  ...
  useEffect(() => {
    ...
    return startMapLifecycle({
      ...
      create: (maplibregl) => {
        // v6 has no WebGL1 fallback. Release this probe before creating the map.
        const context = document.createElement('canvas').getContext('webgl2')
        if (!context) throw new Error('WebGL2 is unavailable')
        context.getExtension('WEBGL_lose_context')?.loseContext()
        if (maplibregl.getVersion() !== mapAssets.version) {
          throw new Error('MapLibre module and worker versions do not match')
        }
        maplibregl.setWorkerUrl(mapAssets.workerUrl)
      ...
      configure: (map, maplibregl, fail) => {
        // A missing tile can recover. Keep the map usable and link to its directory.
        subscriptions.push(map.on('error', (event) => {
        ...
        subscriptions.push(map.on('load', () => setMapWarning(false)))
        ...
        spaces.forEach((space) => {
          ...
          const popupContent = `
            <div class="p-3 text-zinc-200 font-sans max-w-[220px]">
              <span class="text-[10px] font-bold tracking-wider opacity-60">${space.district}</span>
  ...
  return (
    <div className="relative rounded-2xl border border-white/[0.08] bg-[#101012] overflow-hidden">
      ...
      {mapWarning && !mapUnavailable && (
        <p role="status" className="absolute top-3 left-3 right-16 rounded-lg bg-[#101012] px-3 py-2 text-xs text-zinc-300">
          Some map details could not load.{' '}
          <a href="#full-directory" className="text-emerald-400 underline underline-offset-4">Browse the workspace directory</a>.
        </p>
      )}

      {mapUnavailable && (
        <div role="status" className="absolute inset-0 flex flex-col items-center justify-center gap-3 px-6 text-center bg-[#101012]">
          <p className="text-sm text-zinc-300">The interactive map is unavailable.</p>
          <a href="#full-directory" className="text-sm text-emerald-400 underline underline-offset-4 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4">
      ...
      {selectedSpace && (
        <div className="absolute bottom-4 left-4 right-4 md:right-auto md:max-w-md bg-[#0a0a0b]/95 backdrop-blur-md border border-white/[0.08] rounded-xl p-5 shadow-2xl animate-fade-in z-10">
          <div className="flex justify-between items-start">
            <div>
              <span className="text-[10px] font-bold tracking-wider text-emerald-400">{selectedSpace.district}</span>
          ...
          <div className="mt-4 pt-4 border-t border-white/[0.08] flex items-center justify-between">
            <div>
              <span className="text-[10px] text-zinc-500 block tracking-wider">Access model</span>
              <span className="text-sm font-semibold text-zinc-300">{selectedSpace.model}</span>
            </div>
            <div className="text-right">
              <span className="text-[10px] text-zinc-500 block tracking-wider">Price point</span>
